...
torch.hub.set_dir("/home/ttmdawn/fjy_yan2/ToCo_yan2/xiaorong/xiaorong_csc/pretrained/")
...
parser.add_argument("--data_folder", default='/home/ttmdawn/fangjingyuan/data/VOCdevkit/VOC2012/', type=str, help="dataset folder")
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        ### get background crops from uncertain regions ###
        ...
        patch1 = bg_img[:, :, 0:224, 0:224]
        patch2 = bg_img[:, :, 0:224, 224:448]
        patch3 = bg_img[:, :, 224:448, 0:224]
        ...
        ### ptc loss: borrow from https://github.com/rulixiang/ToCo ###
        ...
        ### Discriminative Explicit Alignment (DEA loss) ###
        ...
        ### Confounding Background Decoupling (CBD loss) ###
        ...
        # warmup
        if n_iter <= 2000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + 0.2 * pro_loss + 0.5 * cpc_loss + 0.0 * seg_loss + 0.0 * reg_loss
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + 0.2 * pro_loss + 0.5 * cpc_loss + args.w_seg * seg_loss + args.w_reg * reg_loss
